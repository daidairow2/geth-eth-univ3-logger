...
// 入力ミスの早期検出（ここならOK）
if (THEGRAPH_API_KEY.startsWith('http')) {
  throw new Error('THEGRAPH_API_KEY には「URL」ではなく「キー文字列」だけを入れてください');
}
if (THEGRAPH_SUBGRAPH_ID.includes('/') || THEGRAPH_SUBGRAPH_ID.startsWith('http')) {
  throw new Error('THEGRAPH_SUBGRAPH_ID には ID だけを入れてください（subgraphs/id/ は不要）');
}
